...
import listItemFunctions from './listItemFunctions';
import editListItem from './editItemDOM';

// upload a task to the DOM

const uploadTaskList = function uploadTaskListFunc(taskListArray) {
  ...
  for (let i = 0; i < taskListArray.length; i += 1) {
    ...
    const rowItemCompleteButton = document.createElement('button');
    rowItemCompleteButton.classList.add(taskListArray[i].priority);
    rowItemCompleteButton.addEventListener('click', () => { listItemFunctions.markComplete(taskListArray[i]); });
    ...
    const rowItemDueDate = document.createElement('td');
    rowItemDueDate.classList.add('taskRow', 'taskDate');
    rowItemDueDate.innerHTML = format(taskListArray[i].dueDate, 'MMM dd');
    const today = format(new Date(), 'yyyy-MM-dd');
    const listedDate = format(taskListArray[i].dueDate, 'yyyy-MM-dd');
    if (listedDate > today) {
      rowItemDueDate.classList.add('future');
    ...
    const rowItemProject = document.createElement('td');
    ...
    const rowItemEdit = document.createElement('td');
    rowItemEdit.addEventListener('click', () => { editListItem.loadModal(taskListArray[i]); });
    ...
    const rowItemDelete = document.createElement('td');
    ...
    rowItemDelete.addEventListener('click', () => { listItemFunctions.deleteListItem(taskListArray[i]); });
